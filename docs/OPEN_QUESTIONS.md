...
| **Tip lead: coax or fine wire?** | We chose RG-178, but **ours runs only 1–2 cm before becoming bare copper to the tip holder**, so the shield covers a fraction of the exposed node and buys little. The 2 cm adds only ~2 pF. **The mechanical case against it is stronger:** it lands on the tip holder, which sits on the flexing piezo disc, and bending stiffness goes as diameter⁴. See `docs/UPSTREAM_BERARD.md` §4 | Whether the coax is measurably loading the scanner. **Not on the critical path for the 37 nA** — it cannot cause a DC offset |
| **Is the coax braid grounded, and at which end?** | Nothing recorded | It should be grounded at the **preamp end only**. Both ends is a ground loop; neither end makes the shield useless. One look and one meter check |
