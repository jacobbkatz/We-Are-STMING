...
|---|---|---|
| **Why will the standoff not enter the board hole?** New 2026-09-15, REOPENS a question twice recorded as answered | **`BENCH` 2026-09-15, Jacob: "the standoff doesn't fit in the hole at all."** It did not enter and **nothing was forced or drilled.** This contradicts the "0.076 mm clearance, it drops in" row in `docs/FACTS.md`, whose through-board figure of 2.032 mm came from a **distributor parametric table** — the same class of source that has already been wrong twice on this exact part. **Four candidate causes, each with a cheap test, none yet run:** **(a)** the part in hand is one of the two **11311** we also own, which needs Ø3.45 mm and is already known not to fit — read the DigiKey bag label, and compare the two parts side by side; **(b)** the wrong hole — the standoff hole is the Ø2.108 mm NPTH one nearest the left edge at mid height, and the two Ø2.261 mm mounting holes are **larger**, so a wrong hole would fit **better**, not worse; **(c)** the through-board feature is the **PTFE boss**, not the metal pole, in which case the relevant diameter is nearer Keystone's 2.184 mm recommended hole and **our hole is genuinely undersize** — gauge it by offering the part to a Ø2.261 mm mounting hole on a **bare spare board**, which passes only if the part is between 2.108 and 2.261 mm; **(d)** the hole is obstructed or the fabricated hole is under its 2.108 mm drill size — hold a bare board up to a light. **Do not drill, ream or force anything on the repaired board until this is settled**, and do all fit testing on a bare spare board. | `docs/FACTS.md`, `docs/NEXT_SESSION_PLAN.md` Part D0 |
