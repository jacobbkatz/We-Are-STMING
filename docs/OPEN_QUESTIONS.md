...
| **Which op-amp is actually fitted — OPA627AU or OPA124U?** VERIFY | **New 2026-09-08.** `docs/BOM.md` says **OPA627AU**; the Eagle source we build from says **OPA124U**. **Nobody has read the marking on our own board** — a photo that appeared to settle it turned out to be Dan Berard’s board, not ours. Changes the TIA stability margin (~7x vs ~2x) but **not** the leakage conclusion, since both parts have ~1 pA bias current. **Weight of evidence favours OPA124U** — the Eagle source, Berard’s published schematic and a photograph of **his** assembled board all say OPA124U; the lone `OPA627AU` is `BOM.md`’s, and nobody knows what that CONFIRMED rested on. We build the same board **design** as Berard, but a substitution on our unit is possible and he endorses it. **One minute with a magnifier settles it.** | `docs/COMPONENTS.md` §4 |
