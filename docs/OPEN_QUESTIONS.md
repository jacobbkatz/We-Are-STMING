...
| ~~**What is the Keystone 11301's actual mounting-hole diameter?**~~ **RESOLVED 2026-09-14** | **Ø2.184 mm (0.086").** A distributor parametric listing gives turret diameter 0.080", mount through hole 0.086" × 0.080" — separate figures. **The confusion was that 0.080" is both the head diameter and the below-board length.** Against our 2.108 mm hole that is **0.076 mm interference: a press fit.** Still a distributor table rather than Keystone's own datasheet, so **calipers on the part are the last word** | `docs/FACTS.md` |
| **Is the Keystone 11301 PTFE-insulated, or bare tin-plated brass?** | **Added 2026-09-14, and one look at the part settles it — is there a white plastic collar under the flange?** Distributors disagree: DigiKey's own order line says "TERM TURRET SINGLE L=6.35MM **TIN**" with no insulator, RS calls it "Insulated, PTFE, White". The geometry favours bare: an insulator would have to fit in 0.003" of wall between the 0.080" head and the 0.086" hole. **Berard calls it "a teflon standoff", and if it is actually bare brass then what insulates our input node is washed FR4 and air, not PTFE.** Does not block the build — the standoff hole is NPTH with nothing routed near it — but it changes what the wash is protecting | `docs/UPSTREAM_BERARD.md` §300 |
