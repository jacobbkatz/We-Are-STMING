...
| **Gold leaf — REAL gold, confirmed** | some; a piece is on the sample plate | `SAID` 2026-09-16, Jacob: *"Yes it's real gold leaf"* | **Confirmed real gold, NOT imitation.** Imitation leaf, sold as "Dutch metal", is **86% copper / 14% zinc** and tarnishes; this is not that, so the surface carries **no oxide layer** and stays usable for STM in air. **What was bought had never been recorded** — `sessions/2026-09-16-bench.md` §3.8 noted it as unknown. **Still UNKNOWN: carat, thickness, brand, supplier, and whether it is loose or transfer/patent leaf.** Gold leaf is a fraction of a micron thick, which is why it crumples and clings. **It is currently a crumpled wad in front of the tip, and whether it is electrically joined to the bias has never been metered** — that is step 2 of `docs/NEXT_SESSION_PLAN.md`. **A flat piece burnished onto clean bare copper is a serviceable sample; gilding size is an insulator and would break the bias path** |
| **Copper tape** | some — quantity, width and brand not stated | `SAID` 2026-09-16, Jacob: *"we only have copper tape"* | **CONFIRMED IN THE ROOM.** Corrects this file and `docs/NEXT_SESSION_PLAN.md`, both of which said copper tape was **not owned** as recently as 2026-09-15. Already in use on the sample plate (`sessions/2026-09-16-bench.md` §3.8). **UNKNOWN AND IT MATTERS: whether the adhesive is conductive.** `docs/ENGINEERING_REFERENCE.md` §3 requires conductive adhesive for the shield, and **the same question decides the bias path**: the sample plate is a printed insulator, so if the bias reaches the tip-facing copper only through an insulating adhesive layer, **the sample surface is floating and no tunnelling current can ever flow** — a live candidate for the 2026-09-16 null approach. **Two beeps settle it, powered off:** orange bias wire to the copper top face near the tip (must beep), and copper top face through the adhesive to whatever it is stuck to. **The top face is bare copper, so gold leaf can be burnished directly onto it** |
...
| ~~**Do we own M2 screws, copper tape, heat shrink?**~~ **ALL THREE ANSWERED.** M2 screws and heat shrink ORDERED 2026-09-09; **copper tape CONFIRMED IN THE ROOM 2026-09-16** — see the parts table. **What is still unknown is whether its adhesive conducts** | The shield rebuild needs copper tape with conductive adhesive. Aluminium will not do — `docs/ENGINEERING_REFERENCE.md` §3 |
