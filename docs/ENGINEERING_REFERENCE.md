...
| ADC full scale 10.24 vs 4.096 | PC tools vs firmware driver and schematic | **Resolved: 10.24 V.** ~~Resolved: 4.096. PC tools not yet changed~~ — **that entry was wrong in both halves and is corrected 2026-09-19.** **4.096 V is REFBUF, not the input span**; the LTC2326-16's span is **2.5 x REFBUF = 10.24 V** (`docs/FACTS.md`, measurement chain, and its RETIRED table, which has listed `4.096` as the full scale since 2026-09-07). **And the PC tools HAD already been changed**: `Code/pc/stm_approach.py:118-119` and `:501-504` use 10.24 V and say in the code why. **This row is why the mistake mattered enough to create `docs/FACTS.md`** — the correction of 2026-09-07 reached a dozen documents and left this one, the canonical home for resolved conflicts (`CLAUDE.md` section 6), asserting the retired value as the answer. **`check_facts.py` could not see it**: the line names both numbers, so its correction-table suppression fired. See `deliverables/2026-09-19-pause/LEAD_VERIFICATION.md` V7 |
