"""Figure 13 - we were inside the tunnelling window, and we never parked in it.
...
ABOUT WHETHER TUNNELLING OCCURRED, and it must not be titled as though it were.
...
readings rather than a sampling artefact (14 and 12 readings inside the window, at a 4-count
...
def crossing_stats():
    ...
    """
    import glob
    paths = [S.data_path(SESSION, FILE)] + sorted(
    ...
    for path in paths:
        for v in in_cycles(path).values():
            ...
            if len(inw) < 2:
                continue
            widths.append(abs(inw[-1][0] - inw[0][0]))
            readings += len(inw)
    widths.sort()
...
def main():
    ...
    S.titles_keyed(
        fig,
        "Passing through the tunnelling window is not the same as parking in it",
        ...
        "through.\n<A clean vacuum gap crosses that shelf in 10 to 22 Z counts.> These crossings took %d to %s, "
        "a median of %s."
        % (cs["approaches"], format(cs["readings"], ","), cs["lo"],
    ...
    S.footer(fig, y=0.012, text=
             ...
             "The two narrowest crossings, 52 and 60 counts, are real readings rather than a\n"
             "sampling artefact: 14 and 12 readings inside the window, at a 4-count Z step.")
