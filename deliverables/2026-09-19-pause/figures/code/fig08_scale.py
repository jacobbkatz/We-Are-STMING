...
# (value in amps, label, provenance, emphasize?)
CURRENTS = [
    ...
    (1.0e-9, "ONE NANOAMP — what a tunneling gap would give", "the target", True),
...
# (value in meters, label, provenance, emphasize?)
LENGTHS = [
    (1.5e-10, "1 to 2 ångström — a tunneling current's ten-fold distance",
     "CALC, what tunneling requires", True),
    (1.0e-7, "about 100 nanometers — the gold leaf's thickness",
...
def main():
    ...
    ladder(axc, CURRENTS, 1e-12, 6e4,
           ...
           "Current — each labeled step is a thousand times the last",
           "CURRENT: the half we can put numbers on")

    ladder(axl, LENGTHS, 1e-10, 2e-2,
           [1e-10, 1e-8, 1e-6, 1e-4],
           ["1 ångström", "10 nanometers", "1 micron", "100 microns"],
           "Length — each labeled step is a hundred times the last",
    ...
    S.note(axl, 2.4e-10, -1.85,
           "The 0.016 nanometers per count in circulation was inherited from another "
    ...
    S.titles_keyed(
        ...
        "<One nanoamp> is the current a tunneling gap would give: about a billionth of what a phone charger "
        "delivers, and this instrument resolves it to three picoamps.\nDistances are the other story — the "
        "project can say what a tunneling gap requires, but not what its own scanner does, and it says so rather "
        "than guessing.",
