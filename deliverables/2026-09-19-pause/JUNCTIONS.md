...
tunneling happened — simple and easy to understand, with data and graphs."**
...
> from not-touching to touching without passing through the separations where tunneling is the only
> mechanism available, and bias was applied with the amplifier recording throughout — 60,928
> readings inside that current range, across 109 approaches. **So tunneling happened, and was
> measured.**
>
...
> it reads as though tunneling did not occur, read it as: we could not hold the range.

**Short answer, and it is not the one the question expects.**

> **Two of the three junctions are settled, and neither was a vacuum tunneling gap.**
...
**Almost every argument about tunneling in this project runs through a distance we have never
...
| **Verdict** | **OPEN.** Leans against tunneling | **Contact.** Settled | **Contact.** Settled |
...
**Copper grows a native oxide in air within hours. Gold does not oxidize at all — that is why gold
...
For one motor step to be the 0.1 nm that changes a tunneling current ten-fold, **the lever has to
divide by about 1,550, which puts the tip within 26 micrometers of the pivot line.**
...
| If `d` is | The tip moved, per decade | How far from tunneling |
|---|---|---|
...
**26 micrometers is inside your bound.** It is in the bottom 5% of it, which is why I say the
...
> contradicted tunneling. **You caught it: I had quoted junction C's numbers against junction A.**
...
tunneling needs `d` at the very bottom of the range you measured. **Neither of those is a proof,
...
- **Ten lock-in tests at Z +/-500 all read nothing** (for example −2 +/- 26 counts, +13 +/- 22).
  **Wiggling the tip did not move the current.** In a tunneling gap a +/-500 count wiggle would
...
- **Backing off one motor step at a time: pinned for 23 steps, then clear at step 24, with no
  moderate current in between.** A tunneling current would have fallen smoothly through several
...
- **109 of 110 onsets were gradual.** Not one of the 84 in the bias series was a one-step snap.
  A tunneling current at 4-count Z resolution should arrive almost instantly.
...
100 micrometers of compressible cellulose immediately below the surface we were trying to hold
still to a fraction of a nanometer. **Your own fallback removes it** — *"just putting the gold
...
1. **Junction A's current-voltage curve is a real barrier signature.** Non-ohmic, symmetric,
   ...
   way from nothing.
2. **16 to 59 MΩ is a plausible junction resistance for tunneling.** It is also plausible for a
   poor contact, so it decides nothing, but it is not evidence against.
3. **`d` under 0.5 mm does not exclude 26 micrometers.** Stated again because it is the single
...
**The reason it is still open is that 26 micrometers is below what a straightedge and an eye can
...
**What does resolve it:** a jeweler's loupe or a USB microscope with a scale in the frame, or an
...
**It is worth doing for a second reason.** `d` sets the nanometer scale for every Z measurement
...
You have said you are comfortable calling junction A tunneling and understand the risk. **That is
...
tunneling" as though the data settled it**, because if someone asks the obvious follow-up — *how
...
millimeter and not yet resolved to 26 micrometers. Here is the arithmetic, here is the measurement
