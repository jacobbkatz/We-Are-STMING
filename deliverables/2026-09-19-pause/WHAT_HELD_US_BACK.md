...
junction the noise floor is **9 to 19 counts**, and a person stamping on the floor two meters away
...
6. **the coin mass** — the wrappers stand tall and unrestrained on the platform, raising the center
...
**A tunneling gap is a fraction of a nanometer. Everything in the mechanical path has to be stiff
at that scale.** The sample path was not:

- **Gold leaf sitting on its backing paper** — roughly 50 to 100 micrometers of compressible
...
measurements that were later re-read as the behavior of a tip touching with its side.

**Why tip sharpness is not a detail here.** A tunneling current should come from **one atom**. A
...
sets the lever ratio between the coarse screw and the tip, and therefore nanometers per Z count.
...
is a real constraint and not fine enough — the number that matters is nearer **26 micrometers** and
a straightedge cannot resolve that. **A jeweler's loupe or a USB microscope closes it in minutes.**

---

## 5. The coarse approach cannot stop inside the tunneling window
...
- **`APRH`, the automated approach, was never safe to run.** `approach()` compares `read_adc() >
  target` against a baseline that has been negative all project; if tunneling drives the reading
...
**What needed to change:** settle the sign of the tunneling current so the automatic approach can
...
picoamps, and a real tip-and-sample junction was made and characterized — **the instrument can
measure what it needs to measure.** What it cannot yet do is hold two pieces of metal a
half-nanometer apart for the length of a scan line, because the sample is soft, the tips are blunt,
...
| | Cost | Buys |
|---|---|---|
| Measure `d` with a loupe | minutes, no power | the nanometer scale for every future measurement, and it settles the 2026-09-17 junction retroactively |
