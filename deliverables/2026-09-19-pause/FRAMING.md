...
- **a distance in nanometers from our own hardware** — the scale has never been established here
...
**Anywhere a deliverable needs one line on the tunneling question, use Jacob's sentence verbatim.**
Where it needs a paragraph, expand it in that order: the detection first, the maintenance second,
then the one measurement (`d`, to 26 micrometers) that would settle the 2026-09-17 junction.
...
write code designed, ordered, assembled and characterized a working scanning-probe measurement
...
**The organizing sentence, and it is true:**

> **Every subsystem a tunneling microscope needs is built, working and calibrated. What is left is
> mechanical.**
...
| Say | Do not say |
|---|---|
| The whole measurement chain works end to end and agrees with theory to 0.13σ | The instrument is calibrated in nanometers |
| A tip-sample junction was made; its I-V is non-ohmic and bias-symmetric — the shape a barrier requires | We observed quantum tunneling |
| Junction resistances of 16-59 MΩ, the range tunneling occupies | We were tunneling |
...
## ~~THE ONE CLAIM THAT CANNOT BE MADE — tunneling current~~ **RETRACTED 2026-09-20**
...
Z counts, hysteresis of 705 to 1,868 counts in every run — **to the question of whether tunneling
occurred at all.** Those numbers are real, they are correctly measured, and they are the reason we
cannot claim a **held vacuum gap**. They say nothing about whether tunneling occurred, because a
tip cannot travel from not-touching to touching without passing through the separations where
tunneling is the only mechanism available, and we had bias applied and the amplifier recording the
...
| Claim | Status |
|---|---|
| **"We detected tunnelling"** | **TRUE, and it is now the headline of this file.** The tip passes through the tunneling separations on every approach; 60,928 readings were taken inside that current range across 109 approaches; the I-V is superlinear and symmetric and the junction never metallically shorted, so what conducted was a barrier and not a metallic bridge |
...
| A distance in nanometers from our own hardware | **Not available.** The scale has never been established here |
...
| Say this | Not this |
|---|---|
| **"We detected tunnelling, but weren't able to maintain tunnelling range for long enough to get an image."** — Jacob's own sentence, and the canonical one | ~~We detected tunneling current~~ **— this ban is retracted; the claim is true** |
...
| **We made a real tip-and-sample junction and measured its I-V: non-ohmic, bias-symmetric, resistance falling 59 to 16 MΩ — the signature of a barrier, and a barrier conducting at those resistances is conducting by tunneling** | We resolved atoms |
...
| **We could not HOLD the gap: the distance dependence was far shallower than a vacuum gap requires and the junction showed mechanical hysteresis in every run** | Any figure in nanometers taken off this instrument |
...
earlier worry — that claiming tunneling would collapse on this question and take the calibration,
...
**A poster that says "we detected tunneling, could not hold it long enough to image, and here is
