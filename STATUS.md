...
**Nothing powered, nothing touched.** Jacob sent Dr. Percy Zahl (Brookhaven National Laboratory,
CFN) an update on 2026-09-20 — he advised this project on vibration isolation and scan head design
earlier in the year and is acknowledged in `README.md`. **He replied on 2026-09-22.** Full account
and the point-by-point answer in `sessions/2026-09-22.md`; the drafted reply is
`deliverables/2026-09-19-pause/report/REPLY_TO_ZAHL.md`, **unsent**.

| | |
|---|---|
| **HIS HEADLINE: *"Not surprising."*** | **He did not treat the drift as a puzzle.** His instruction was to isolate systematically and eliminate the biggest contributor first: monitor temperature precisely, enclose, thermalize for hours, look for mismatched expansion coefficients in the tip-sample loop, and **look up piezo creep** |
| **PIEZO CREEP IS BACK ON THE LIST, AND IT HAD FALLEN OFF** | It was on the 2026-09-17 candidate list in `docs/OPEN_QUESTIONS.md` and **was not among the seven candidates this file carried**. The list now runs to **EIGHT**. **It matters more than an eighth candidate normally would, because THE PIEZO IS ALSO THE RULER**: the 43,000 counts is the Z position at which a current threshold is crossed during a piezo sweep, so a scanner whose displacement-per-count is drifting gives that reading with a perfectly still gap. **`docs/ENGINEERING_REFERENCE.md` §12: our scanner's travel, linearity, hysteresis and creep have NEVER been measured** |
| **BUT OUR OWN DATA ARGUES AGAINST TEXTBOOK CREEP FOR THE FAST EXCURSION** | Creep decays one way. **The 6.4 s excursion reverses and overshoots its own starting point** (`deliverables/2026-09-19-pause/LEAD_VERIFICATION.md` V6). **So creep-after-a-step does not fit it.** Not excluded for the slower motions, and his second half — joints and stressed points creeping *for a long time* — lands on candidates 2 and 7, a twisted-rubber-band retention and a spring-hook-in-eyebolt joint |
| **A NEW, CHEAP TEST WE HAD NOT THOUGHT OF** | **Is the 705-1,868 counts of in/out hysteresis the junction, or the PIEZO'S OWN hysteresis?** It has only ever been read as "a soft, pressed, sticky contact". **The scanner fits one fact the contact reading does not explain by itself: the hysteresis shows NO trend with bias**, and a scanner is indifferent to sample bias. **Discriminator: piezo hysteresis scales with sweep amplitude, contact compliance does not.** Run the in/out Z cycle at full, half and quarter amplitude. **Now STEP 5b in the plan.** No purchase, and it needs only a repeatable contact |
| **STEP 1 REWRITTEN: HOURS, NOT FIFTEEN MINUTES, AND LOG THE TEMPERATURE** | The box test was a 15-minute recording. **Zahl: enclose and *"let it thermalize for a few hours"*.** And **this project has never recorded the room temperature beside a measurement** — no thermometer in `docs/INVENTORY.md`, no temperature row in `docs/FACTS.md`. **ANSWERED THE SAME SESSION — `SAID` Jacob: *"no we dont have any temp monitoring"*. WE OWN NONE, so this is now a PURCHASE and it is the only thing on the bench plan that needs one.** What to buy: **0.01 to 0.1 K resolution, logging unattended, standalone with no electrical connection to the instrument** — **not** a sensor wired to the Teensy, whose data line would toggle beside a 100 MOhm input node. `docs/NEXT_SESSION_PLAN.md` STEP 1(b) |
| **WHAT WE COULD ALREADY ANSWER, AND IT IS MOST OF WHAT HE ASKED** | **Six of his seven questions are answered in this repository.** 3D printed parts in the tip-sample loop: **yes, essentially all of it**. Soft materials: **three, and one is 50-100 um of backing paper directly under the gold**. Parts under spring load: **three twisted rubber bands, the suspension springs, and the spring-hook-in-eyebolt joints**. Micrometer drive: **no** — hand-turned 1/4-80 screws at 0.88 um/degree plus a 28BYJ-48 on a screw at 155 nm/step divided by a lever over 80 |
| **ELECTRICAL STABILITY: ANSWERED WITH A NUMBER, AND IT CLEARS THE ELECTRONICS** | He asked whether the signals are stable in time. **Tip clear, Z parked, the amplifier read −24 to +16 counts over 8.0 minutes** (`sessions/2026-09-19-morning.md` §3.2) — about ±75 pA against the ~1 nA we hunt. **At ~1,800 Z counts per decade and a ~940 pA threshold, that offset can shift the crossing point by about 60 counts. Against 43,000 observed.** **CALC 2026-09-22 from our own measured numbers** |
| **THE THERMAL ARITHMETIC IS A BRACKET, NOT A MEASUREMENT** | Every 10 mm of unmatched PETG-CF in the loop uses up **our entire Z range (under ~0.51 um) for roughly half a kelvin to a couple of kelvin**. **Our filament's expansion coefficient is not in `docs/FACTS.md`, its datasheet could not be fetched** (network policy blocks the manufacturer and the distributor mirrors), **and the length of printed material in the loop has never been measured.** **No CTE figure has been written into this repository** — a search snippet is not a datasheet, and this project took three wrong web answers in a row on the Keystone standoff |
| **PETG-CF RE-CONFIRMED** | Jacob doubted it mid-session, then withdrew the doubt: *"sorry it is petg-cf"*. **A second confirmation eleven days after the first.** The five slicer files still select PA-CF and always will — they record the abandoned attempt, the switch happened at the printer, and `CAD/prints/README.md` has the banner. **Recorded in `docs/INVENTORY.md` so it is not searched for a third time** |
| **ONE RECORD DEFECT FIXED** | `deliverables/2026-09-19-pause/WHAT_HELD_US_BACK.md` still carried *"at least 1.94 nm"* in two places, the inversion flagged on 2026-09-21. **Grepped the class: those were the only two live occurrences.** **And flipping the word alone would have left a broken argument**, because "under 1.94 nm against a 0.17 nm window" does not show the motor overshooting. **It now rests on the bench observation instead** — single steps shifted the surface 14,000-29,000 counts when they shifted it at all, runs of 20-40 did nothing |
| **THE PUBLIC PAGE NOW SAYS WHAT ITS OWN APPEAL PRODUCED** | Its closing section asks anyone who has chased drift at this scale to say how they found it. **Zahl had, and the page did not know.** It now carries his advice, **paraphrased and credited, with no verbatim quotes — it was private correspondence, and a source comment says not to add any without asking him.** **Two stale figures went with it and BOTH were classes:** *"seven candidates"* and *"a fifteen-minute recording"* each appeared **twice**. **`check_site.py` gained `check_candidate_count`**, which found the second *"seven candidates"* that a by-hand pass had missed; **proven red on four variants and green after.** Page passes all twelve markup checks and the browser audit at five widths in both themes |
| **THE PROJECT HAS BEEN REACHING OUT MORE WIDELY THAN THIS REPOSITORY KNEW** | Read from Jacob's Gmail 2026-09-22 with his permission, and **now recorded in the new [`docs/OUTREACH.md`](docs/OUTREACH.md)**. **Four technical letters went out on 2026-09-21**, all leading with the corrected central claim and all linking the public page. **`docs/OUTREACH.md` is the canonical home for who was asked, when, and what is still pending** |
| **A PENDING EXPERT REPLY NOBODY HAD RECORDED: DR. XIAO TONG** | **He manages the STMs at Brookhaven's CFN** — the most STM-specific expert this project has reached. Ming Lu forwarded the drift problem to him on 2026-09-21 and warned the government fiscal year ends first: **no feedback expected before 1 October 2026.** **Do not chase before October; do check after it.** **Nothing in the bench plan waits on him** — but if he answers before the rebuild, read his ranking before STEP 1 runs. `docs/OPEN_QUESTIONS.md` |
| **THE DRAFT REPLY TO ZAHL WAS SUPERSEDED BEFORE IT WAS WRITTEN, AND THAT IS WHY `OUTREACH.md` EXISTS** | **Jacob sent his own reply at 16:52 UTC, about an hour before the draft was finished**, covering the same ground. **The draft is bannered SUPERSEDED, not deleted**, because **two questions in it were never asked and are still worth asking**: how to separate the scanner's own creep from real gap motion with no position sensor, and whether a mostly-printed tip-sample loop is disqualifying or an engineering problem. **And one courtesy is outstanding: the public page now names him and he has not been told** |
| **A SECOND REPLY, AND IT IS A MATERIALS RULING RATHER THAN A RANKING** | Asked what to look at first, the Brookhaven adviser **did not rank our candidates — he ruled out materials.** **No plastics of any kind, no rubber of any kind, no paper** (paper *goes crazy* with changing humidity); squishy materials **flow under pressure essentially forever** at this scale. Use **metals** (aluminium noted for its high expansion coefficient) and **ceramic, alumina, glass, sapphire, granite, quartz or mica** for insulation. A screw drive is fine **if every thread is steel or brass and it is spring pre-loaded against backlash**; **steel balls at any pivot.** **THREE OF OUR LOOP'S MATERIALS ARE ON HIS NO LIST — the printed plates, the rubber bands and the backing paper — and all three sit in this file as candidates to be TESTED.** `docs/OUTREACH.md`, `docs/NEXT_SESSION_PLAN.md` **STEP 2b** |
| **THE ONE IDEA NOBODY HERE HAD: MAKE THE LOOP SYMMETRIC** | **Expansion then cancels.** With tip and sample dead centre a symmetric frame has **zero XY thermal drift**, and in Z only the *difference* between frame, piezo, tip-wire length and sample thickness remains. **It is the cheap half of the materials problem** — it buys thermal immunity without changing a single material |
| **AND A MECHANISM THAT DEFEATS OUR OWN BOX TEST** | **Paper tracks HUMIDITY.** A box controls temperature and draughts; **it does not control humidity**, which moves slowly and reverses — the exact signature we are chasing. **So a clean box test does NOT clear the backing paper.** Written into STEP 1 as a caveat on what a pass means |
| **THE ADVISER IS NO LONGER NAMED ON THE PUBLIC PAGE** | Jacob's instruction, 2026-09-22, after the name had been live about an hour. The page now says **a physicist at Brookhaven National Laboratory**. **`check_site.py` gained `check_adviser_unnamed`**, which fails on his first name, surname or username anywhere in the page — **proven red on four variants.** **He is still named in `docs/OUTREACH.md`**, which is where follow-ups are tracked. **Note this repository is public and his name remains in its working documents and history** — Jacob's call, not closed |
| **THE WEBSITE HAD A FULL EDITORIAL PASS, AND IT FOUND A BUG IN ITS OWN JAVASCRIPT** | **Visible text down 21.5%** with nothing deleted — 1,049 words behind expandables, **all 71 links, 18 figures and every anchor verified intact.** Narrative reordered so the imaging attempt sits between the tunneling result and the failure analysis. **The price comparison no longer leads**; an engineering scoreboard does. **THE BUG: the tile count-up used `parseInt`, which stops at a comma** — `parseInt("60,928")` is 60, and it wrote back a bare integer, so the number animated to the wrong value and lost its separator. **Latent until now because every animated number had been comma-free.** Fixed and verified. **"We detected tunneling" is unchanged and unhedged**, checked mechanically |
...
| **THE TIPS WERE ETCHED, NOT CUT — AND THEY WERE SHARP WHEN FITTED** | `SAID` Jacob, 2026-09-21: *"We used etched tips the whole time"*, and, asked to choose between readings, that they were **sharp when fitted and blunted in use**. **This answers items (4) and (5) of the etching row in `docs/INVENTORY.md`, open since 2026-09-19**, and makes `README.md`'s *"cut tungsten tip, 0.25 mm wire cut at 45°"* wrong — corrected. **What it changes about the blockers:** tip bluntness is **not an independent blocker**, it is a consequence of the instrument snapping into contact — that tip was pressed into the gold **four separate times in one morning, twice for over half an hour**. **Fix the approach and the tip stays sharp.** **STILL UNKNOWN and still needed:** the NaOH concentration, the voltage and the counter-electrode — nobody can repeat the etch from this repository. ~~**NOT RECONCILED**~~ **RESOLVED the same session, and it changes the blocker: *"they were all hand-measured and independent voltages, not good science, that's why they weren't so sharp — they changed sharpness depending on run."*** **THE ETCH WAS NEVER CONTROLLED**, so a tip could arrive blunt AND a sharp one could be flattened in use; the bench logs' *"very blunt"* for the 2026-09-19 tip is simply one of the bad runs. **THE TIP IS AN UNCONTROLLED VARIABLE IN EVERY RESULT THIS PROJECT HAS.** **Until one voltage, one concentration and one counter-electrode are fixed and written down, a failed run cannot be attributed to the tip or to the instrument** — which makes fixing the etch the cheaper half AND the one that makes the other half measurable |
| **THE SAMPLE-PLATE BAND COUNT IS SETTLED, AND THE ASYMMETRY IS THE 2026-09-17 FIX** | `SAID` Jacob, 2026-09-21: **3 bands, each twisted 3 times — ONE across the screw pair by the scan head, TWO across the pair by the motor.** **This reconciles a contested count**: a photo reading of the 2026-09-19 frames said two, `sessions/2026-09-17-bench.md` §17 records one more added at the bench. Both saw correctly; neither had the distribution. **Why it matters: §149 diagnosed that both original bands pulled IN LINE with the two side-by-side ball ends — the line the plate tips around — so the motor end lifted off its screw.** Putting the extra band at the motor end moves the combined pull inside the triangle the three contacts make. **New row in `docs/INVENTORY.md`, which had none** |
| **WHAT DID NOT CHANGE** | **No measurement, no safety rule, no fault, no stage verdict.** The instrument is where 2026-09-19 left it |
...
| | |
|---|---|
| **WHERE IT IS: BEING DISASSEMBLED AND MOVED** | `SAID`, Jacob, ~14:19 UTC 2026-09-19: *"We are now taking it apart and moving it to News House as I'm flying to San Diego for college. We will probably continue a little bit less frequently over the next few months."* **"News House" is read as Nuh's house** (speech-to-text) — an inference, not confirmed. **What was taken apart and how it was packed is UNKNOWN.** ~~Parked for the break: side screws 2 turns back, covered~~ — superseded by the move. **Reassemble and re-verify before any power** (`docs/NEXT_SESSION_PLAN.md`, top block) |
...
| **THE GAP DOES NOT HOLD STILL** | **With the motor and hands still** (piezo sweeping only): ≥ 43,000 counts in 6.4 s, ≥ 56,000 over ~2 min (12:54-12:56). During scans: ≥ 48,000 in 33 s. **No period evident** (the data are too sparse to exclude one). **Cause UNKNOWN — ~~four~~ SEVEN untested candidates, expanded 2026-09-19 from the photograph review: the leaf on its paper; the plate on its bands and balls; thermal motion; air currents; **(5) THE UNDRESSED CABLES** — an orange lead and a four-way bundle cross from the suspended platform to the fixed bench and **no frame shows either clamped, taped or tied to a strain-relief point**. A suspended stage is only as soft as its softest path to ground and a cable IS a path to ground; worse, it is a nonlinear one that can stick to the bench and release. **Test: hang the platform, add a known small mass, measure the droop with the cables connected and again with them lifted clear**; **(6) THE COIN MASS** — the wrappers stand tall and unrestrained on the platform, raising the centre of gravity and free to slide; **(7) THE SPRING-HOOK-IN-EYEBOLT JOINTS.** **PLUS a fifth candidate for the 6.4 s excursion specifically: POST-MOTOR-MOVE RELAXATION** — that window opens 0.1 s after a 60-step retract ended, though the motion reverses and overshoots its start, which relaxation does not do (`deliverables/2026-09-19-pause/LEAD_VERIFICATION.md` V6). **None of 5-7 is evidence: a photograph shows what is touching what, never what moved.** `deliverables/2026-09-19-pause/photos/MECHANICAL_OBSERVATIONS.md`. **The instrument is now disassembled, so each must be re-checked at reassembly rather than assumed to have survived the move** |
...
| **Imaging** | **No image — and the conclusion is unchanged, but ~~the controls reproduce better (image to image +0.37 against +0.04)~~ IS WITHDRAWN, corrected 2026-09-19 pause-point.** **`cas9_xheld2.csv` is a SINGLE SCAN LINE, not an image** — the run aborted after line one, where the other seven files carry eleven. `scripts/analyze_scans.py` lines 50-51 truncate both images to the shorter one with no shape check, so **two of the three control correlations were computed over 21 points instead of 231**, comparing one line against another image's first line. **Like for like, full images only: scans +0.09, +0.18, −0.15 (mean +0.04); controls +0.20, −0.06 (mean +0.07). Indistinguishable, and neither differs from zero.** **The right statement is simpler and needs no correction to be believed: neither the scans nor the controls reproduce, and the scans show no more reproducible structure than a control in which the tip never moved across the surface.** The published means were reproduced exactly before the cause was found; see `deliverables/2026-09-19-pause/LEAD_VERIFICATION.md` V4. **Three-Y control**: at 3,000 apart the same shape everywhere (the scanner); at 12,000 one run said "surface" at 3.9σ and **the repeat did not**. Wide images: trace/retrace **−0.75 to −0.94** ~~(loop lag)~~ **corrected 2026-09-19 morning: the loop HUNTING, not lag — each line is detrended first, so no drift ramp can make it; X motion the likely, unproven cause** (`sessions/2026-09-19-morning.md` §5), saturation ~15%. **ALSO WITHDRAWN 2026-09-19 pause-point: the wide images' ~~image to image +0.56 to +0.86~~ figure.** Two of the three wide scans aborted (`img_scan_0.csv` is 1 line, `img_scan_1.csv` is 2, against 11), so those are 21- and 42-point fragment correlations, **not** image-to-image. **With one complete real wide image there is no valid reproducibility figure for the wide scans at all.** The controls' **+0.56 over 231 points IS sound** and is a real result: **with X held the instrument repeats its own pattern strongly.** `LEAD_VERIFICATION.md` V5 |
