...
> # SPENT PLAN — this session happened on 2026-08-31. NOT a current instruction.
>
> Marked 2026-09-08. **This is history: the plan a session was run against.** Its results are in
> `sessions/2026-08-31-results.md`, which supersedes it.
>
> **Do not follow it.** Among other things it predates the 119 nA measurement, the corrected
> 10.24 V ADC full scale, the floating `PREAMP−` finding, and the discovery that the preamp box is
> full of cyanoacrylate. Current work is `docs/NEXT_SESSION_PLAN.md`.
>
> **One correction it contains is still cited elsewhere:** its dummy-junction resistor advice was
> wrong (it suggested 1 MΩ). The corrected figure lives in `STATUS.md` and `docs/FACTS.md`.
